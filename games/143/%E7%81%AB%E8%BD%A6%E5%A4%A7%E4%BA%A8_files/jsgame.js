...
(function() {
		...
		o = {
			...
			loadImages: function(b, c) {
				...
				var d = jsGame.canvas.screen.getWidth(),
					g = jsGame.canvas.screen.getHeight(),
					l = (d - 200) / 2,
					s = g - 40;
				b = parseInt(a.image.reCountLoaded) > b ? parseInt(a.image.reCountLoaded) : b;
				b = b > c ? c : b;
				jsGame.canvas.fillStyle(a.canvas.bgColor).fillRect(0, 0, d, g).strokeRect(l, s, 200, 5).fillStyle("#FFFFFF").fillRect(l + 1, s + 1, b / c * 198, 3);
				if (a.image.loadedImg) {
					...
					for (l = 0; l < a.image.loadFrame.length; l++) {
						jsGame.canvas.drawImage(a.image.loadImgId, 47 + a.image.loadFrame[l].frames[a.image.loadFrame[l].step++] * 7, 3, 7, 7, d + a.image.loadFrame[l].x, g + a.image.loadFrame[l].y, 7, 7);
						a.image.loadFrame[l].step %= a.image.loadFrame[l].frames.length
					}
				} else jsGame.canvas.drawString("加载中", 0, parseInt(g / 2), jsGame.graphics.VCENTER, true, "#FFFFFF", "#000000");
	...
	jsGame = {
		...
		objExtend: function() {
			...
			for (; c < d; c++) if ((l = arguments[c]) != null) for (var s in l) {
				var r = b[s],
					p = l[s];
				if (b !== p) if (g && p && typeof p === "object" && !p.nodeType) b[s] = this.objExtend(g, r || (p.length != null ? [] : {}), p);
				else if (p !== undefined) b[s] = p
			}
			return b
		},
		getJson: function(b) {
		...
		commandFuns: function() {
			...
			return {
				...
				inArray: function(c, d) {
					var g, l = d.length;
					for (g = 0; g < l; g++) if (c == d[g]) return g;
					return -1
				},
				collisionCheck: function(c, d, g, l, s, r, p, v) {
					if (p && Math.abs(c + parseInt(g / 2) - (s + parseInt(p / 2))) < parseInt((g + p) / 2) && Math.abs(d + parseInt(l / 2) - (r + parseInt(v / 2))) < parseInt((l + v) / 2)) return true;
					return false
				},
				circleCollisionCheck: function(c, d, g, l, s, r) {
					c = Math.abs(c - l);
					d = Math.abs(d - s);
					if (Math.sqrt(c * c + d * d) < g + r) return true;
					return false
				}
			}
		}(),
		...
		canvas: function() {
			...
			return {
				...
				pass: function(e, h, j) {
					var i;
					i = !e || e == "" ? a.canvas.defaultId : e;
					if (!a.canvas.ctxs[i]) {
						e = e ? document.createElement("canvas") : b.base().getDom(i);
						a.canvas.ctxs[i] = e.getContext("2d");
						e.width = h ? h : a.canvas.defaultWidth;
						...
						a.canvas.cavansDoms[i] = e
					}
					c = a.canvas.ctxs[i];
					c.font = a.canvas.defaultFont;
					p = a.canvas.cavansDoms[i];
					w = parseInt(p.width);
					x = parseInt(p.height);
					return b.screen.setId(i)
				},
				...
				fillRect: function(e, h, j, i, k) {
					j = j ? j : 0;
					i = i ? i : 0;
					if (k) s = o.getAnchor(e, h, j, i, k);
					else {
						s.x = e;
						s.y = h
					}
					c.fillRect(s.x, s.y, j, i);
					return b
				},
				fillText: function(e, h, j, i) {
					c.font = i || a.canvas.defaultFont;
					c.fillText(e, h, j);
					return b
				},
				clearRect: function(e, h, j, i) {
					c.clearRect(e, h, j, i);
					return b
				},
				...
				strokeRect: function(e, h, j, i, k) {
					if (k) l = o.getAnchor(e, h, j, i, k);
					else {
						l.x = e;
						l.y = h
					}
					c.strokeRect(l.x, l.y, j, i);
					return b
				},
				strokeText: function(e, h, j, i) {
					c.font = i || a.canvas.defaultFont;
					c.strokeText(e, h, j);
					return b
				},
				setColor: function(e, h, j) {
					if (j == null) {
						g.fillColor = e;
						g.strokeColor = h ? h : e
					} else {
						g.fillColor = "rgb(" + e + ", " + h + ", " + j + ")";
						g.strokeColor = g.fillColor
					}
					return b.fillStyle(g.fillColor).strokeStyle(g.strokeColor)
				},
				drawImage: function(e, h, j, i, k, q, m, n, t, u) {
					if (i == null) c.drawImage(jsGame.getImage(e), h, j);
					else if (k == null) {
						d = o.getAnchor(h, j, jsGame.getImage(e).width, jsGame.getImage(e).height, i);
						c.drawImage(jsGame.getImage(e), d.x, d.y)
					} else if (u == null) c.drawImage(jsGame.getImage(e), h, j, i, k, q, m, n, t);
					else {
						d = o.getAnchor(q, m, n, t, u);
						c.drawImage(jsGame.getImage(e), h, j, i, k, d.x, d.y, n, t)
					}
					return b
				},
				drawCache: function(e, h, j, i, k, q, m, n, t, u) {
					if (e = a.canvas.cavansDoms[e]) if (i == null) c.drawImage(e, h, j);
					else if (k == null) {
						d = o.getAnchor(h, j, e.width, e.height, i);
						c.drawImage(e, d.x, d.y)
					} else if (u == null) c.drawImage(e, h, j, i, k, q, m, n, t);
					else {
						d = o.getAnchor(q, m, n, t, u);
						c.drawImage(e, h, j, i, k, d.x, d.y, n, t)
					}
					return b
				},
				drawRegion: function(e, h, j, i, k, q, m, n) {
					switch (q) {
					default:
						c.setTransform(1, 0, 0, 1, m, n);
						break;
					case f.canvas.trans.TRANS_ROT90:
						c.setTransform(0, 1, -1, 0, k + m, n);
						break;
					case f.canvas.trans.TRANS_ROT180:
						c.setTransform(-1, 0, 0, -1, i + m, k + n);
						break;
					case f.canvas.trans.TRANS_ROT270:
						c.setTransform(0, -1, 1, 0, m, i + n);
						break;
					case f.canvas.trans.TRANS_MIRROR:
						c.setTransform(-1, 0, 0, 1, i + m, n);
						break;
					case f.canvas.trans.TRANS_MIRROR_ROT90:
						c.setTransform(0, -1, -1, 0, k + m, i + n);
						break;
					case f.canvas.trans.TRANS_MIRROR_ROT180:
						c.setTransform(1, 0, 0, -1, m, k + n);
						break;
					case f.canvas.trans.TRANS_MIRROR_ROT270:
						c.setTransform(0, 1, 1, 0, m, n)
					}
					b.drawImage(e, h, j, i, k, 0, 0, i, k);
					c.setTransform(1, 0, 0, 1, 0, 0);
					return b
				},
				drawNumber: function(e, h, j, i, k, q, m, n, t) {
					e = e.toString();
					var u = e.length;
					n = n ? n : j;
					t = t ? t : i;
					if (m) for (m = 0; m < u; m++) b.drawImage(h, parseInt(e.charAt(m)) * j, 0, j, i, k + m * n, q, n, t);
					else for (m = u - 1; m >= 0; m--) b.drawImage(h, parseInt(e.charAt(m)) * j, 0, j, i, k - (u - 1 - m) * n, q, n, t, jsGame.graphics.ANCHOR_RT);
					return b
				},
				moveTo: function(e, h) {
					c.moveTo(e, h);
				...
				arc: function(e, h, j, i, k, q) {
					c.arc(e, h, j, i, k, q);
					return b
				},
				quadraticCurveTo: function(e, h, j, i) {
					c.quadraticCurveTo(e, h, j, i);
					return b
				},
				bezierCurveTo: function(e, h, j, i, k, q) {
					c.bezierCurveTo(e, h, j, i, k, q);
					return b
				},
				measureText: function(e) {
					var h = c.measureText(e),
						j = h.width;
				...
				drawLine: function(e, h, j, i) {
					return b.beginPath().moveTo(e, h).lineTo(j, i).closePath().stroke()
				},
				drawRect: function(e, h, j, i, k) {
					return b.strokeRect(e, h, j, i, k)
				},
				drawString: function(e, h, j, i, k, q, m, n) {
					r.x = h;
					r.y = j;
					...
					case f.canvas.graphics.VCENTER:
						r.x = parseInt(b.screen.getWidth() - b.measureText(e).width >> 1);
						break;
					case f.canvas.graphics.RIGHT:
						r.x = b.screen.getWidth() - b.measureText(e).width
					}
					if (k) {
						r.fillStyle = q ? q : "#000000";
						r.strokeStyle = m ? m : "#CCCCCC";
						b.fillStyle(r.strokeStyle).fillText(e, r.x + 1, r.y + 1, n).fillStyle(r.fillStyle)
					}
					return b.fillText(e, r.x, r.y, n).fillStyle(a.canvas.defaultColor)
				},
				drawSubstring: function(e, h, j, i, k, q, m, n, t, u) {
					return b.drawString(e.substring(h, h + j), i, k, q, m, n, t, u)
				},
				clip: function() {
					c.clip();
					return b
				...
				setTransform: function(e, h, j, i, k, q) {
					c.setTransform(e, h, j, i, k, q);
					return b
				},
				scale: function(e, h) {
					c.scale(e, h);
		...
		loadImage: function(b) {
			if (a.system.gameFlow != f.system.gameFlowType.loadImage && b.length > 0) {
		...
		delImage: function(b) {
			if (a.image.imgs[b]) a.image.imgs[b] = null;
			return this
		},
		audio: function() {
			var b = null;
		...
		initAudio: function(b) {
			if (b.length > 0) {
				...
				for (var c = 0; c < b.length; c++) {
					a.audio.audios[b[c].id] = {
						id: b[c].id,
						dom: this.getDom(b[c].id),
						src: b[c].src,
						isPlaying: false
					};
